		...
		spawn(0)
			for(var/mob/living/M in stomach_contents)
				if(M.loc != src)
					stomach_contents.Remove(M)
					continue
				if(istype(M, /mob/living/carbon) && stat != 2 && stendo != 1)
					if(M.stat == 2)
						M.death(1)
						stomach_contents.Remove(M)
						src << "<span class='notice'>You feel [M] gurgle away...</span>"
		...
		spawn(0)
			for(var/mob/living/M in cock_contents)
				if(M.loc != src)
					...
					continue
				if(istype(M, /mob/living/carbon) && stat != 2 && cvendo != 1)
					if(M.stat == 2)
						cockfull = 1
						M.death(1)
						cock_contents.Remove(M)
						src << "<span class='notice'>You feel [M] dissolve into cum...</span>"
						qdel (M)
						continue
					if(air_master.current_cycle%3==1)
						if(!(M.status_flags & GODMODE))
		...
		spawn(0)
			for(var/mob/living/M in boob_contents)
				if(M.loc != src)
					...
					continue
				if(istype(M, /mob/living/carbon) && stat != 2 && bvendo != 1)
					if(M.stat == 2)
						boobfull = 1
						M.death(1)
						boob_contents.Remove(M)
						src << "<span class='notice'>You feel [M] dissolve into milk...</span>"
						qdel (M)
						continue
					if(air_master.current_cycle%3==1)
						if(!(M.status_flags & GODMODE))
			...
			for(var/mob/living/M in womb_contents)
				...
				if(istype(M, /mob/living/carbon) && stat != 2 && wombheal == "Digest")
					if(M.stat == 2)
						...
						M.death(1)
						womb_contents.Remove(M)
						src << "<span class='notice'>You feel [M] dissolve into femcum...</span>"
						qdel (M)
						continue
					if(air_master.current_cycle%3==1)
						if(!(M.status_flags & GODMODE))
						...
						if(!(M.status_flags & GODMODE))
							...
							if(TFchance == 1)
								...
								if(TFmodify == 1)
									...
									//P.g_eyes = src.g_eyes
									//P.b_eyes = src.b_eyes
									P << "<span class='notice'>You feel very strange...</span>"
									P.update_body()
								if(TFmodify == 2)
									//P.r_hair = src.r_hair
