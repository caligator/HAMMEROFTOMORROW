...
/mob/living/carbon/human/proc/update_body()
	...
	//Skin
	var/image/img_skin = image("icon"='icons/mob/human.dmi', "icon_state"="[base_icon_state]_s", "layer"= -BODY_LAYER)
var
	H = 19 //hue
	S = 47 //saturation
	L = s_tone //lightness
...
if (L < 0 || L > 1)
	L = 1
skin_c = (1-abs(2L-1))*S
skin_x = skin_c * (1-abs((H/60)%2-1))
skin_m = L - C/2
switch(round(H/60,1))
	if(0)
		img_skin.color = rgb(skin_c+m, skin_x+m, 0+m)
	if(1)
		img_skin.color = rgb(skin_x+m, skin_c+m, 0+m)
	if(2)
		img_skin.color = rgb(0+m, skin_c+m, skin_x+m)
	if(3)
		img_skin.color = rgb(0+m, skin_x+m, skin_c+m)
	if(4)
		img_skin.color = rgb(skin_x+m, 0+m, skin_c+m)
	if(5)
		img_skin.color = rgb(skin_c+m, 0+m, skin_x+m)
	img_skin.blend_mode = BLEND_ADD
