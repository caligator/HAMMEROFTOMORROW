	...
	proc/ShowChoices(mob/user)
		...
		switch(current_tab)
			if (0) // Character Settings#
				...
				dat += "Skin Tinge: <BR><a href='?_src_=prefs;preference=s_tinge;task=input'>[-s_tinge + 35]/220<br></a>" //Grafted by String
		...
		switch(href_list["task"])
			...
			if("input")
				switch(href_list["preference"])
					...
					if("mutant_race")
						var/new_mutant_race = input(user, "Choose your character's species:", "Character Preference")  as null|anything in mutant_races
						if(new_mutant_race)
					...
					if("s_tinge")
						var/new_s_tone = input(user, "Choose your character's skin-tinge:\n(Light 1 - 220 Dark)", "Character Preference")  as num|null
						if(new_s_tone)
							s_tinge = 35 - max(min( round(new_s_tone), 220),1)
